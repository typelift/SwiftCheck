...
//            return pf(AnySequence<Element>(bl.map(wit))) as! (AnySequence<(_)>)
			return Swift._undefined()
        })
    }
}
...
extension Repeated where Element : Arbitrary {
    ...
    public static var arbitrary : Gen<Repeated<Element>> {
		...
		return Gen<(Element, Int)>.zip(Element.arbitrary, Int.arbitrary).map { args in let (e, i) = args; return constructor(e, i) }
    }
}
